...
namespace _20180628_SP.v1
{
  public partial class FrmSenadores : Form
  {
    ...
    public void ManejadorVoto(string senador, Votacion.EVoto voto)
    {
      ...
      else
      {
        ...
        if (aux == 0)
        {
          ...
          try
          {            
            if (dao.Guardar("", this.votacion))
            {
              MessageBox.Show("Pudo guardarse correctamente");
            }            
          }
          catch (Exception e)
          {
            MessageBox.Show("No pudo guardarse en SQL.\n"+e.Message);
          }

          try
          {
            SerializarXML<Votacion> serializer = new SerializarXML<Votacion>();
            serializer.Guardar(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop),"xmlfile.xml"), this.votacion);
          }
          catch (Exception e)
          {
            MessageBox.Show("No pudo guardarse en XML.\n" + e.Message);
          }
        }
      }
    }
    ...
    private void btnSimular_Click(object sender, EventArgs e)
    {
      ...
      // THREAD
      thread = new Thread(this.votacion.Simular);
      thread.Start();
    }
